...
1. **Check if a subagent is still running** — call `agent_query` with `names: ["agent_name"]` to get `finished` status without waiting.
...
```json
[{"name":"agent1","starting_time":"...","finished":false,"oneShot":false}]
```
