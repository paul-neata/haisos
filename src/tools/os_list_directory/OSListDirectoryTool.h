...
public:
    ...
    // CLAUDE.md).
    static std::shared_ptr<OSListDirectoryTool> Create(std::shared_ptr<CurrentProcessHandle> process) {
        return std::shared_ptr<OSListDirectoryTool>(new OSListDirectoryTool(std::move(process)));
    }
...
private:
    explicit OSListDirectoryTool(std::shared_ptr<CurrentProcessHandle> process);
