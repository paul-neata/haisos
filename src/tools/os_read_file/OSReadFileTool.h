...
public:
    ...
    // CLAUDE.md).
    static std::shared_ptr<OSReadFileTool> Create(std::shared_ptr<CurrentProcessHandle> process) {
        return std::shared_ptr<OSReadFileTool>(new OSReadFileTool(std::move(process)));
    }
...
private:
    explicit OSReadFileTool(std::shared_ptr<CurrentProcessHandle> process);
