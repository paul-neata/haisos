...
public:
    ...
    // CLAUDE.md).
    static std::shared_ptr<OSWriteFileTool> Create(std::shared_ptr<CurrentProcessHandle> process) {
        return std::shared_ptr<OSWriteFileTool>(new OSWriteFileTool(std::move(process)));
    }
...
private:
    explicit OSWriteFileTool(std::shared_ptr<CurrentProcessHandle> process);
